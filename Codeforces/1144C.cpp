...
signed main()
	{
		...
		REP(i, n) {
			cin >> u;
			if(vis[u] == 0) vs.pb(u), vis[u] = 1;
			else if(vis[u] == 1) vs2.pb(u), vis[u] = 2;
			else ansx = 0;
		}
		...
		if(!ansx) cout << "NO" << endl;
			...
			sort(vs2.begin(), vs2.end(), greater <int>());
			cout << "YES" << endl;
			cout << vs.size() << endl; REP(i, vs.size()) cout << vs[i] << " "; cout << endl;
			cout << vs2.size() << endl; REP(i, vs2.size()) cout << vs2[i] << " "; cout << endl;
