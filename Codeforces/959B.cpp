...
signed main()
	{
		...
		REP(i, k) {
			cin >> num;
			vi v;
			REP(j, num) cin >> x, x--, v.pb(x);
			int mval = INT_MAX;
			for(auto it : v) mval = min(mval, a[it]);
			for(auto it : v) mx[vs[it]] = mval;
		}

		int ans = 0;

		REP(i, m) cin >> s, vs2.pb(s);
		for(auto it : vs2) ans += mx[it];
