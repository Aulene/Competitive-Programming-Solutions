...
signed main()
	{
		...
		int num_reqd = ceil((ld) n * prob / 100.0);
		...
		// for(i = 0; i < n; i++) cout << vs[i].x << " " << vs[i].y << endl;

		while(clock() < 3.9 * CLOCKS_PER_SEC) {
			u = rng() % n, v = rng() % n;
			...
			Line <int> l = makeLine(p1, p2);

			int num = 0;

			for(i = 0; i < n; i++)
				if(onLine(vs[i], l)) num++;
